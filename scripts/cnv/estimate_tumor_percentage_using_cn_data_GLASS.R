...
# library(ggplot)
# 
# tpc.estimate.tp <- tpc.estimate %>%
#   dplyr::filter(grepl("-TP$",sample.short)) %>%
#   `colnames<-`(paste0(colnames(.), ".TP")) %>%
#   dplyr::mutate(pid = gsub('^([^-]+-[^-]+-[^-]+)-.+$', '\\1', sample.short.TP) )
#   
# 
# tpc.estimate.tr <- tpc.estimate %>%
#   dplyr::filter(grepl("-TP$",sample.short) == F )  %>%
#   `colnames<-`(paste0(colnames(.), ".TR")) %>%
#   dplyr::mutate(pid = gsub('^([^-]+-[^-]+-[^-]+)-.+$', '\\1', sample.short.TR) )
# 
# 
# tpc.estimate.paired <- dplyr::inner_join(tpc.estimate.tp , tpc.estimate.tr, by = c('pid'='pid'))
# 
# 
# tpc.estimate <- tpc.estimate %>%
#   dplyr::mutate(resection.status = grepl("-TP", sample.short)) %>%
#   dplyr::mutate(resection.status = ifelse(resection.status, "primary", "recurrence"))
# 
# 
# 
# m1 <- tpc.estimate %>%
#   dplyr::filter(!is.na(pct) & resection.status == "primary") %>%
#   dplyr::pull(pct) %>%
#   median()
# 
# m2 <- tpc.estimate %>%
#   dplyr::filter(!is.na(pct) & resection.status == "recurrence") %>%
#   dplyr::pull(pct) %>%
#   median()
# 
# 
# ggplot(tpc.estimate %>% dplyr::filter(grepl("TCGA",sample.short)==F), aes(x = resection.status, y = pct, col=resection.status)) +
#   geom_violin() +
...
# 
# 
# wilcox.test(tpc.estimate %>%
#               dplyr::filter(grepl("TCGA",sample.short) ) %>% 
#               dplyr::filter(!is.na(pct) & resection.status == "primary") %>%
#               dplyr::pull(pct) ,
#             tpc.estimate %>%
#               dplyr::filter(grepl("TCGA",sample.short) ) %>% 
