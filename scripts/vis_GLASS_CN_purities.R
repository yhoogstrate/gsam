...
render_glass <- function(bc) {
  # print(bc)
  # bc = "GLSS-CU-R004-TP-01D-WXS-46GQ36"
  ...
  purity <- purities |>
    dplyr::filter(.data$aliquot_barcode.wgs == bc)
  ...
  ggplot(plt, aes(x = pos, y = log2_copy_ratio, group = id, col = chrom)) +
    ...
    labs(
      x = NULL, y = "log2 copy ratio Synapse portal",
      caption = paste0("aliquot_barcode W[X/G]S: ", bc, "  -  purity estimate: ", purity * 100, "%")
    ) +
    scale_x_continuous(breaks = c(0, 50, 100, 150, 200, 250, 300))

  ggsave(paste0("output/figures/cnv/glass/2022/", bc, "_estimate_gg.pdf"), width = 8.3, height = 3.5, scale = 1.75)
}



pbapply::pblapply(unique(dat$aliquot_barcode), render_glass)
