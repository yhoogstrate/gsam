...
with open("/tmp/run_alignment_star_part3.sh", "w") as fh:
	fh.write("#!/bin/bash\n\n")
	
	for __ in sorted([_ for _ in os.listdir("fastq-clean") if len(_) == 4]):
		for _ in os.listdir("fastq-clean/" + __):
			
			sid = _.split("-",2)[1].split("_")[0]
		...
		for sid in sorted(idx[pid]):
			...
			for srid in sorted(idx[pid][sid]):
				if idx[pid][sid][srid]['R1'] is not None and idx[pid][sid][srid]['R2'] is not None:
					r1.append('fastq-clean/' + idx[pid][sid][srid]['R1'])
					r2.append('fastq-clean/' + idx[pid][sid][srid]['R2'])
					#print("    " + srid + "\t\tCOMPLETE")
				else:
			...
			fh.write('    --alignIntronMax 1000000 \\\n')
			fh.write('    --alignMatesGapMax 1000000 \\\n')
			...
			fh.write('    --outFilterIntronMotifs RemoveNoncanonicalUnannotated \\\n')
			fh.write('    --quantMode TranscriptomeSAM\n')
