...
stopifnot(is.na(glass.gbm.rnaseq.metadata.all.samples$portion_barcode) == F)
...
glass.gbm.rnaseq.metadata.all.samples <- glass.gbm.rnaseq.metadata.all.samples |> 
  dplyr::mutate(excluded = ifelse(aliquot_barcode %in% c(
    "GLSS-CU-R004-TP-01R-RNA-U0DEP1",
...
tmp <- read.table('data/gsam/data/GLASS_GBM_R1-R2/GLASS.analysis_mut_freq.syn32908224.tsv',sep="\t",header=T,stringsAsFactors = F) |> 
