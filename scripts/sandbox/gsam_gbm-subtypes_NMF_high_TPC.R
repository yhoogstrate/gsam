...
# check if samples w/ bad quality are excluded
stopifnot(gsam.rna.metadata %>% dplyr::filter(blacklist.pca) %>% dplyr::pull(sid) %in% colnames(tmp) == F)
...
metadata <- gsam.rna.metadata %>%
  ...
  dplyr::mutate( #needs to be calculutated from this source code
    `NMF.123456.membership` = NULL,
    `NMF.123456.PC1` = NULL,
    `NMF.123456.PC2` = NULL,
    `NMF.123456.PC3` = NULL,
    `NMF.123456.PCA.LDA.class` = NULL,
    `NMF.123456.PCA.LDA.posterior.Classical` = NULL,
    `NMF.123456.PCA.LDA.posterior.Mesenchymal` = NULL,
    `NMF.123456.PCA.LDA.posterior.Proneural` = NULL ) %>%
  dplyr::filter(!is.na(tumour.percentage.dna) & tumour.percentage.dna >= 13)
...
stopifnot(colnames(tmp) == metadata$sid)
...
if(!file.exists("tmp/gsam_nmf_150_high_tpc.Rds")) {
  gsam_nmf_150 <- {{}}
  ...
  gsam_nmf_150 <- readRDS("tmp/gsam_nmf_150_high_tpc.Rds")
}
...
plt <- plt.single %>%
  ...
  dplyr::mutate(tpc = ifelse(tumour.percentage.dna >=13 & tumour.percentage.dna <= 15 , "TPC <= 13", "TPC > 13") ) %>%
  dplyr::mutate(label = paste0(pid,".",tumour.percentage.dna) )
...
ggplot(plt, aes(x = `NMF:123456.PC1`, y = `NMF:123456.PC2`, group=pid,
                col = `NMF:123456.PCA.LDA.status` , label=label)) + 
  geom_raster(data = nmf.pca.lda.countours %>% dplyr::mutate(tpc = NA, pid=NA, label=NA, `NMF:123456.PCA.LDA.status`=NA) , aes(fill = factor(class)), alpha=0.1) +
  geom_contour(data=  nmf.pca.lda.countours %>% dplyr::mutate(tpc = NA, pid=NA , label=NA, `NMF:123456.PCA.LDA.status`=NA) , aes(z=as.numeric(class)),
               colour="gray40", size=0.25, lty=2,breaks=c(1.5,2.5)) +
  ...
  geom_point(data = plt %>% dplyr::filter(grepl("<",tpc )), size=2.5 , col="red") +
  geom_path(
    data = plt %>% dplyr::filter(pid %in% .$pid[duplicated(.$pid)]) %>%
      dplyr::filter( low.tpc.pair  ),
    arrow = arrow(ends = "last", type = "closed", angle=15, length = unit(0.125, "inches"))
  ) + 
  geom_text_repel(data = plt %>% dplyr::filter(low.tpc.pair == T), size = 3, col="purple")  +
  youri_gg_theme +
  labs(x="PC1 on NMF meta-features", y="PC2 on NMF meta-features", col=NULL,fill="LDA class") +
  scale_color_manual(values=c('Stable'= rgb(0,0.75,0), 'Transition' = 'black','NA'='purple')) +
  scale_fill_manual(values = subtype_colors)
...
plot(plt$`NMF:123456.PC1` , plt$tumour.percentage.dna)
plot(plt$`NMF:123456.PC2` , plt$tumour.percentage.dna)
