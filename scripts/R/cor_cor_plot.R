...
cor_cor_plot <- function(normalised_correlated_data) {
  normalised_correlated_data <- tmp.bak
  
  # remove duplicate entries:
  plt <- normalised_correlated_data %>%
    tibble::rownames_to_column('__hugo_symbol__') %>%
    dplyr::filter(!duplicated(`__hugo_symbol__`)) %>%
    dplyr::mutate(`__hugo_symbol__` = gsub("^ENSG[0-9\\._]+","", `__hugo_symbol__`)) %>%
    dplyr::mutate(`__hugo_symbol__` = gsub("chr.+$","", `__hugo_symbol__`)) %>%
    dplyr::mutate(`__hugo_symbol__` = gsub('|','', `__hugo_symbol__`,fixed=T)) %>%
  ...
  p1 <- ggplot(plt.expanded2,
    ...
    ggplot2::coord_fixed() +
    scale_y_continuous(name=NULL, breaks = 1:length(o), labels = o)
  
  
  
  plt <- data.frame(gid = o, i = 1:length(o)) %>%
    dplyr::left_join(labels.bak %>% tibble::rownames_to_column('gid'), by=c('gid' = 'gid')) %>%
    reshape2::melt(id.vars = c('gid','i'))  
  
  
  p2 <- ggplot(plt, aes(x = reorder(gid, -i) , y = variable, fill=value, label=gid)) +
